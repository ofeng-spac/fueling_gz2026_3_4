...
from robot_control.robot_connection import RobotClient
...
def test_get_pose(robot_client):
    """测试获取机器人位姿"""
    print("\n=== 测试获取机器人位姿 ===")
    try:
        pose = robot_client.get_robot_pose()
        if pose:
            print(f"当前位姿:")
            print(pose)
...
def test_move_to_target_pose(robot_client, target_pose, pose_name):
    """测试移动到目标位姿"""
    print(f"\n=== 测试移动到 {pose_name} ===")
    print(f"目标位置:")
...
def main():
    ...
    try:
        ...
        robot_client.connect(ROBOT_IP, ROBOT_PORT)
        print("✓ 机器人连接成功")
    except Exception as e:
        print(f"❌ 机器人连接失败: {e}")
        print("请检查:")
    ...
    try:
        # 测试1: 获取当前位姿
        initial_pose = test_get_pose(robot_client)
        if not initial_pose:
        ...
        for pose_name, target_pose in poses_to_test.items():
            ...
            if response.lower() not in ['y', 'yes']:
                ...
                continue
            # success = test_move_to_target_pose(robot_client, target_pose, pose_name)
            success = False
            ...
            if success:
                ...
                # 验证是否到达目标
                final_pose = test_get_pose(robot_client)
                if final_pose:
                    # 计算位置误差
