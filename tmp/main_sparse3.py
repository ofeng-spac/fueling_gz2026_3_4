...
from fueling.stereo_matcher.drawing import vis_disparity
...
async def run_fuel(arm_id: int, stereo_matcher: StereoMatcherService,
                   ...
                   source_pc: o3d.geometry.PointCloud, initial_disparity_path: str, minima_matcher_service: MinimaMatcherService):
    """
    Main function to run the end-to-end camera capture and inference pipeline.
    方案4: 使用视差图进行MINIMA匹配，只需投影一次。

    Args:
        arm_id: Unique identifier for this task (0-3)
        initial_disparity_path: 初始化时保存的视差图pkl文件路径
    """
    # ========== 配置参数提取 ==========
    ...
    # ========== 加载视差图并转换为3通道可视化图像 ==========
    logger.info(f"Task {arm_id}: 加载视差图: {initial_disparity_path}")

    # 从pkl加载原始float视差值（不丢精度）
    with open(initial_disparity_path, 'rb') as f:
        initial_disparity_raw = pickle.load(f)

    logger.info(f"Task {arm_id}: 视差图原始形状: {initial_disparity_raw.shape}, dtype: {initial_disparity_raw.dtype}")
    logger.info(f"Task {arm_id}: 视差图范围: {np.min(initial_disparity_raw[initial_disparity_raw > 0]):.2f} - {np.max(initial_disparity_raw):.2f}")

    # 使用vis_disparity转换为3通道BGR可视化图像（INFERNO colormap）
    initial_disparity_vis = vis_disparity(initial_disparity_raw)
    logger.info(f"Task {arm_id}: 视差图可视化后形状: {initial_disparity_vis.shape}, dtype: {initial_disparity_vis.dtype}")

    # 保存可视化的视差图用于调试
    if debug_mode:
        disparity_vis_path = minima_output_directory / 'initial_disparity_vis.png'
        cv2.imwrite(str(disparity_vis_path), initial_disparity_vis)
        logger.info(f"Task {arm_id}: 保存可视化视差图到: {disparity_vis_path}")

    # ========== 步骤1: 计算初始投影边框（只投影一次到视差图） ==========
    logger.info(f"Task {arm_id}: 步骤1 - 将源点云投影到视差图上，确定初始ROI（只需投影一次）...")
    K_mat = np.array(config['robot']['RS_camera']['K']).reshape(3, 3)
    from fueling.minima.geometry import project_pointcloud_to_image_float

    # 将源点云投影到视差图上（使用左相机内参，因为视差图是基于左图的）
    if debug_mode:
        projected_img_path_disp = os.path.join(minima_output_directory, "projected_source_on_disparity.png")
        point_arr_disp, indices_disp, bbox_disp = project_pointcloud_to_image_float(
            source_pc, initial_disparity_vis, K_mat, projected_img_path_disp, None
        )
    else:
        point_arr_disp, indices_disp, bbox_disp = project_pointcloud_to_image_float(
            source_pc, initial_disparity_vis, K_mat, None, None
        )

    # 记录边框信息
    if bbox_disp is not None:
        disp_width = bbox_disp[2] - bbox_disp[0]
        disp_height = bbox_disp[3] - bbox_disp[1]
        logger.info(f"Task {arm_id}: 源点云在视差图上的投影边框: [{bbox_disp[0]:.1f}, {bbox_disp[1]:.1f}, {bbox_disp[2]:.1f}, {bbox_disp[3]:.1f}] 大小: {disp_width:.1f}x{disp_height:.1f}")
    else:
        logger.error(f"Task {arm_id}: 无法计算视差图上的投影边框")
        return

    # 保存带有初始边框的视差图
    if debug_mode:
        initial_disp_with_bbox = create_bbox_image(initial_disparity_vis, bbox_disp)
        cv2.imwrite(str(images_output_directory / 'initial_disparity_with_bbox.png'), initial_disp_with_bbox)
    ...
    # ========== 步骤2: MINIMA匹配（视差图与当前左右图） ==========
    logger.info(f"Task {arm_id}: 步骤2 - MINIMA匹配（视差图 vs 当前左右图）...")
    ...
    logger.info(f"Task {arm_id}: 视差图与左图匹配点数量: {len(mkpts0_full_left)}")

    # 第二次匹配：视差图 vs 当前右图
    match_res_right = await minima_matcher_service.match(initial_disparity_vis, images['right_ir'])
    mkpts0_full_right, mkpts1_full_right, mconf_right = match_res_right['mkpts0'], match_res_right['mkpts1'], match_res_right['mconf']
    logger.info(f"Task {arm_id}: 视差图与右图匹配点数量: {len(mkpts0_full_right)}")

    # ========== 步骤3: 过滤匹配点并计算当前图像ROI边框 ==========
    logger.info(f"Task {arm_id}: 步骤3 - 过滤匹配点并计算当前图像ROI边框...")

    # 过滤左图匹配点（只保留视差图上在初始边框内的点）
    mkpts0_filtered_left, mkpts1_filtered_left = filter_matches_by_bbox(
        mkpts0_full_left, mkpts1_full_left, bbox_disp
    )
    ...
    mkpts0_filtered_right, mkpts1_filtered_right = filter_matches_by_bbox(
        mkpts0_full_right, mkpts1_full_right, bbox_disp
    )
    ...
    # 注意：这里使用ROI坐标系下的点
    # 左图：视差图上的匹配点 vs 当前点云在左图上的投影点
    intersection_results_kdtree_left = compute_bidirectional_intersection_kdtree(
        mkpts0_filtered_left, mkpts1_roi_left, point_arr_disp, point_arr2_left_roi, tolerance=1.0
    )
    ...
    if debug_mode and len(both_idx_left) > 0:
        ...
        visualize_matches(initial_disparity_vis, images['left_ir'],
                         mkpts0_filtered_left_final, mkpts1_full_coords,
                         str(minima_output_directory), "matches_left_roi")

    # 右图：视差图上的匹配点 vs 当前点云在右图上的投影点
    intersection_results_kdtree_right = compute_bidirectional_intersection_kdtree(
        mkpts0_filtered_right, mkpts1_roi_right, point_arr_disp, point_arr2_right_roi, tolerance=1.0
    )
    ...
    if debug_mode and len(both_idx_right) > 0:
        ...
        visualize_matches(initial_disparity_vis, images['right_ir'],
    ...
    sparse_pc1_left, sparse_pc2_left = create_sparse_pointclouds_from_bidirectional_matches_float(
        ...
        point_arr_disp, point_arr2_left_roi,
        indices_disp, indices2_left,
    ...
    sparse_pc1_right, sparse_pc2_right = create_sparse_pointclouds_from_bidirectional_matches_float(
        source_pc, current_pc1,
        point_arr_disp, point_arr2_right_roi,
        indices_disp, indices2_right,
    ...
    if debug_mode:
        report = {
            "arm_id": arm_id,
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "method": "Method 4: Disparity-based ROI (single projection)",
            "initial_bbox": {
                "disparity": list(map(int, bbox_disp)) if bbox_disp is not None else None,
            },
            "current_roi_bbox": {
            ...
            "image_sizes": {
                "initial_disparity": list(initial_disparity_vis.shape),
    ...
    logger.info(f"Task {arm_id}: 方案4: 基于视差图的ROI pipeline测试完成")
...
async def main():
    ...
    for i, config_file in enumerate(config_files):
        ...
        # 加载视差图路径 - 从配置文件中读取或使用默认路径
        if 'minima' in config and 'disparity_path' in config['minima']:
            disparity_path = config['minima']['disparity_path']
        else:
            # 默认路径：使用初始化时保存的视差图
            disparity_path = f"fueling/initialization/output_disparity/orbbec/Flood_light/arm{i+1}/{target_pot}/left_disp.pkl"

        # 如果路径是相对路径，转换为绝对路径
        if not os.path.isabs(disparity_path):
            disparity_path = os.path.join(proj_dir, disparity_path)

        # 检查文件是否存在
        if not os.path.exists(disparity_path):
            logger.error(f"Disparity file not found at {disparity_path}")
            raise FileNotFoundError(f"Disparity file not found: {disparity_path}")

        logger.info(f"Loaded disparity path for arm{i+1}: {disparity_path}")
        initial_disparity_paths.append(disparity_path)

    async def run_with_delay(i, delay):
        if delay > 0:
            logger.info(f"机械臂 {i} 延迟 {delay}s 启动")
            await anyio.sleep(delay)
        await run_fuel(i, stereo_matcher, robot_clients[i], pc_registrations[i], configs[i], source_pcs[i], initial_disparity_paths[i], minima_matcher_service)
