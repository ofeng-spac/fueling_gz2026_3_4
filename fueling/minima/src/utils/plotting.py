...
def make_matching_figure(
        ...
        kpts0=None, kpts1=None, text=[], dpi=300, path=None,svg=False, draw_lines=True):
    # draw image pair
    ...
    if mkpts0.shape[0] != 0 and mkpts1.shape[0] != 0:
        ...
        # step = max(1, len(mkpts0) // 300)          # 保留约 300 个点，每隔 step 个取 1 个点
        # idx  = np.arange(0, len(mkpts0), step)
        # mkpts0, mkpts1, color = mkpts0[idx], mkpts1[idx], color[idx]

        # if draw_lines: # 筛选之后画横线
        #     fig.lines = [matplotlib.lines.Line2D((fkpts0[i, 0], fkpts1[i, 0]),
        #                                         (fkpts0[i, 1], fkpts1[i, 1]),
        #                                         transform=fig.transFigure, c=color[i], linewidth=1)
        #                                     for i in range(len(mkpts0))]

        s, alpha = 2, 0.6          # s 散点直径 alpha 透明度 想再稀疏就 1, 0.4
        axes[0].scatter(mkpts0[:, 0], mkpts0[:, 1], c=color, s=s, alpha=alpha)
        axes[1].scatter(mkpts1[:, 0], mkpts1[:, 1], c=color, s=s, alpha=alpha)

    # put txts
    txt_color = 'k' if img0[:100, :200].mean() > 200 else 'w'
    fig.text(
        0.01, 0.99, '\n'.join(text), transform=fig.axes[0].transAxes,
        fontsize=18, va='top', ha='left', color=txt_color)
    ...
    if path:
        if not svg:
            ...
            svg_path = path.replace('.jpg', '.pdf')
            from matplotlib import rcParams
            rcParams['svg.fonttype'] = 'none'  # 'none'
            plt.savefig(svg_path, bbox_inches='tight', pad_inches=0)
    else:
        return fig

def make_matching_figure2(
        img0, img1, mkpts0, mkpts1, color,
        kpts0=None, kpts1=None, text=[], dpi=300, path=None,svg=False):
    # draw image pair
    assert mkpts0.shape[0] == mkpts1.shape[0], f'mkpts0: {mkpts0.shape[0]} v.s. mkpts1: {mkpts1.shape[0]}'
    mkpts0 = mkpts0.astype(np.int32)
    mkpts1 = mkpts1.astype(np.int32)
    ...
    if mkpts0.shape[0] != 0 and mkpts1.shape[0] != 0:
        ...
        fig.lines = [matplotlib.lines.Line2D((fkpts0[i, 0], fkpts1[i, 0]),
                                            (fkpts0[i, 1], fkpts1[i, 1]),
                                            transform=fig.transFigure, c=tuple(color[i]), linewidth=1)
                                        for i in range(len(mkpts0))]
